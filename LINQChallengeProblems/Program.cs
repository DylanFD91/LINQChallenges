    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            */
            ...
            classGrades.ForEach(m => m.Split(',').ToArray().Select(s => int.Parse(s)).OrderByDescending(h => h).Select(n => n != m.Min()).ToList().Average(x => m.Length));
            ...
            string example = "Danielle";
            example.ToCharArray();
            char[] answer = example.Distinct().Count();
